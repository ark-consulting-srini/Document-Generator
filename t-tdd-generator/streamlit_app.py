#!/usr/bin/env python3
"""
T - TDD Generator v2.2 (Multi-Platform Architecture)

Main Streamlit application entry point.

Features:
1. Parse Informatica XML mapping AND workflow files
2. Generate Conceptual, Logical, and Physical data models
3. Multi-platform LLM integration (Snowflake Cortex / Databricks Foundation Models)
...
from ui.tabs.tab_dbt_generator import render_dbt_generator_tab
...
def main():
    ...
    # ---- Sidebar: Target Platform Selector ----
    with st.sidebar:
        st.markdown("### Target Platform")
        target_platform = st.radio(
            "Generate SQL for:",
            options=["snowflake", "databricks"],
            format_func=lambda x: {"snowflake": "Snowflake", "databricks": "Databricks (Spark SQL)"}[x],
            index=0 if st.session_state.get('target_platform', 'snowflake') == 'snowflake' else 1,
        ...
        if st.button("Load Sample Data", key="load_sample_btn",
                     help="Load sample XML files from sample_xmls/ for quick testing"):
            _load_sample_data()

        st.markdown("---")
        st.markdown("### Optional Tabs")
        st.caption("Enable LLM-heavy or advanced tabs as needed.")
        st.checkbox("Conversion Report", key="enable_conversion_report",
                     help="LLM-powered Informatica→cloud conversion analysis")
        st.checkbox("SQL Generator", key="enable_sql_generator", value=True,
                     help="Generate DDL/DML/Stored Procedures from mappings")
        st.checkbox("dbt Models", key="enable_dbt_models",
                     help="Generate dbt bronze + silver models")

    st.title(f"{APP_ICON} {APP_TITLE}")
    st.markdown("""
    Upload Informatica XML files (mappings and/or workflows) to automatically generate:
    ...
    uploaded_files = st.file_uploader(
        "Upload Informatica XML or SQL files",
        type=['xml', 'XML', 'sql', 'SQL'],
        accept_multiple_files=True,
        help="Upload mapping XMLs (m_*.XML), workflow XMLs (wf_*.XML), or SQL files (.sql)",
    ...
    if sample_files and not uploaded_files:
        ...
        st.info(f"Loaded {len(uploaded_files)} sample XML file(s) from sample_xmls/")
    ...
    if uploaded_files:
        ...
        elif st.button("🚀 Parse XML and Generate Documentation", type="primary"):
            parse_and_process_files(uploaded_files)
    else:
    ...
    if st.session_state.get('parsed_data'):
        # Clear files button — lets user start fresh with different XMLs
...
def render_configuration_section():
    """Render the configuration section for TDD generation."""
    st.markdown("---")
    st.markdown("### 📝 TDD Configuration")
    ...
    with col_m1:
        ...
        st.session_state.use_primary = st.checkbox("Use Primary", value=True, key="use_primary_cb")
    ...
    # TDD Generation Mode
    st.markdown("### 📊 TDD Generation Mode")
...
def parse_and_process_files(uploaded_files):
    ...
    for file_idx, uploaded_file in enumerate(uploaded_files):
        ...
        try:
            ...
            # ---- Route .sql files to the SQL lineage parser ----
            if fname_lower.endswith('.sql'):
                status_text.text(f"Parsing SQL file: {uploaded_file.name}...")
                sql_parser = SQLLineageParser(file_content, filename=uploaded_file.name)
                ...
                sql_lineage = sql_parser.build_lineage(
                ...
                if sql_lineage:
                    ...
                    all_sources.extend(sql_parser.sources_data)
                    all_targets.extend(sql_parser.targets_data)
                    all_sources_dict.update(sql_parser.sources)
                    all_targets_dict.update(sql_parser.targets)
                    all_mappings_dict.update(sql_parser.mappings)
                    mappings_from_standalone.update(sql_parser.mappings.keys())
                else:
                    st.warning(f"No lineage extracted from {uploaded_file.name}. "
                               "The file may contain only DDL (CREATE TABLE) or unsupported SQL.")
                continue  # skip XML parsing for .sql files
...
def render_results_tabs():
    ...
    if st.session_state.get('enable_dbt_models', False):
        tab_entries.append(("dbt Models", lambda: render_dbt_generator_tab(
            sources_dict=data['sources_dict'],
            targets_dict=data['targets_dict'],
            lineage_df=data['df_lineage'],
            mappings_dict=data.get('mappings_dict', {}),
        ...
        if 'error' in tdd_results:
            ...
            if 'connection failed' in err_msg.lower() or 'secrets.toml' in err_msg.lower() or 'not configured' in err_msg.lower():
                st.info(
                    ...
                    "token = \"dapi...\"\n```\n\n"
                    "All other tabs (STTM, SQL Generator, Lineage Diagrams, dbt) work without credentials."
                )
            else:
                st.error(err_msg)
        else:
...
def render_enhanced_sttm_tab(data):
    ...
    if lineage_df is None or lineage_df.empty:
        st.info("No lineage data available. Parse XML files first.")
        return
        ...
        if not LLM_PLATFORM_AVAILABLE:
            ...
            if not conn:
                ...
                with st.spinner("Analyzing requirements and updating STTM..."):
                    ...
                    tp = st.session_state.get('target_platform', 'snowflake')
                    platform_label = "Snowflake SQL" if tp == 'snowflake' else "Databricks Spark SQL"
                    date_trunc_example = "DATE_TRUNC('MONTH', CURRENT_DATE())" if tp == 'snowflake' else "TRUNC(CURRENT_DATE(), 'MONTH')"
...
def render_instructions():
    ...
    st.info("Upload Informatica XML files (mappings or workflows) to begin")
    
    with st.expander("How to Use"):
        st.markdown("""
        **Step 1:** Export your Informatica mappings and/or workflows as XML files

        **Step 2:** Upload one or more XML files using the uploader
        - Mapping XMLs (m_*.XML) - data transformation logic
        - Workflow XMLs (wf_*.XML) - orchestration, sessions, dependencies

        **Step 3:** (Optional) Enter Business Context and Requirements

        **Step 4:** Select LLM models for TDD generation

        **Step 5:** Click "Parse XML and Generate Documentation"

        **Step 6:** Explore the tabs:
        - **TDD**: AI-generated Technical Design Document
        - **Workflows**: Orchestration overview (if workflow XML uploaded)
        - **Task DAG**: Visual execution dependency graph
        - **Sessions**: Session configs, connections, pre/post SQL
        - **Sources/Targets**: Table definitions
        - **STTM**: Source-to-Target Mapping
        - **Data Model**: Visual diagrams
        - **SQL Generator**: Convert to SQL
        """)
