"""
SQL Generator Module for T - TDD Generator

Converts Informatica XML mappings to SQL statements for Snowflake or Databricks:
...
- Stored Procedures / Databricks Notebooks: Complete ETL with error handling
"""
...
class ColumnDefinition:
    ...
    def to_ddl(self, target_platform: str = 'snowflake') -> str:
...
class TableDefinition:
    ...
    def to_ddl(self, target_platform: str = 'snowflake', if_not_exists: bool = True) -> str:
...
{columns_sql}
    ...
    def to_snowflake_ddl(self, if_not_exists: bool = True) -> str:
        return self.to_ddl('snowflake', if_not_exists)


class SQLGenerator:
    """
    Main SQL Generator class for converting Informatica XML to SQL.
    ...
    def generate_insert_sql(self, target_table: str,
        ...
        for _, row in target_lineage.iterrows():
            ...
            if expression:
                ...
                if source_col and source_col.startswith("'") and source_col.endswith("'"):
                    ...
                    if self.target_platform == 'databricks':
                        source_expressions.append(f"    MONOTONICALLY_INCREASING_ID() AS {target_col}  -- Sequence")
                    else:
                        source_expressions.append(f"    SEQ_{target_table}.NEXTVAL AS {target_col}  -- Sequence")
                elif clean_col:
        ...
        if len(source_tables_list) == 0:
            if self.target_platform == 'databricks':
                ...
                from_clause = "FROM DUAL  -- No actual source tables found; check lookups above"
        ...
        # Build SQL
        platform_label = "Databricks Spark SQL" if self.target_platform == 'databricks' else "Snowflake"
        sql_parts = [
    ...
    def generate_merge_sql(self, target_table: str,
        ...
        if len(source_tables_list) == 0:
            if self.target_platform == 'databricks':
                ...
                from_clause = f"FROM DUAL  -- No actual source tables"
        elif len(source_tables_list) == 1:
            table = source_tables_list[0]
            alias = table_aliases[table]
            from_clause = f"FROM {source_schema}.{table} {alias}"
...
WHEN NOT MATCHED THEN INSERT (
    ...
    def generate_stored_procedure(self, procedure_name: str,
        ...
        if self.target_platform == 'databricks':
            templates = get_sql_templates('databricks')
        ...
        return SQL_TEMPLATES['stored_procedure'].format(
            schema=schema,
            procedure_name=procedure_name,
            etl_logic=dml_indented
        )
...
def map_datatype(informatica_type: str,
    ...
    if target_platform == 'databricks':
        ...
        if mapped_type in ['VARCHAR', 'CHAR', 'BINARY', 'VARBINARY']:
            if precision:
                return f"{mapped_type}({precision})"
            return mapped_type
        elif mapped_type in ['NUMBER', 'NUMERIC', 'DECIMAL']:
            if precision and scale:
                return f"{mapped_type}({precision},{scale})"
            elif precision:
                return f"{mapped_type}({precision})"
...
def map_datatype_to_snowflake(informatica_type: str,
                              precision: str = None,
                              scale: str = None) -> str:
    """Map Informatica datatype to Snowflake datatype (backward-compat wrapper)."""
    return map_datatype(informatica_type, precision, scale, 'snowflake')
...
def convert_informatica_expression(expression: str,
    ...
    - Function conversions (IIF -> IFF/IF, etc.)
    ...
    # ===========================================
    is_databricks = (target_platform == 'databricks')
    ...
    # Handle IIF -> IFF (Snowflake) or IF (Databricks)
    iif_target = 'IF(' if is_databricks else 'IFF('
    result = re.sub(r'\bIIF\s*\(', iif_target, result, flags=re.IGNORECASE)

    # Handle ISNULL with two args -> NVL (Snowflake) or COALESCE (Databricks)
    if is_databricks:
        result = re.sub(r'\bISNULL\s*\(\s*([^,)]+)\s*,\s*([^)]+)\s*\)',
                        r'COALESCE(\1, \2)', result, flags=re.IGNORECASE)
    else:
        result = re.sub(r'\bISNULL\s*\(\s*([^,)]+)\s*,\s*([^)]+)\s*\)',
                        r'NVL(\1, \2)', result, flags=re.IGNORECASE)

    # Handle single-arg ISNULL in IFF/IF context
    cond_func = 'IF' if is_databricks else 'IFF'
    result = re.sub(
        rf'\b{cond_func}\s*\(\s*isnull\s*\(\s*([^)]+)\s*\)',
        lambda m: f'{cond_func}({m.group(1).strip()} IS NULL',
        result, flags=re.IGNORECASE
    )

    # Handle standalone isnull(x) -> x IS NULL
    def convert_isnull_condition(match):
        arg = match.group(1).strip()
        return f"{arg} IS NULL"

    result = re.sub(r'\bisnull\s*\(\s*([^,)]+)\s*\)',
                    convert_isnull_condition, result, flags=re.IGNORECASE)

    # Handle NVL -> COALESCE on Databricks
    if is_databricks:
        result = re.sub(r'\bNVL\s*\(', 'COALESCE(', result, flags=re.IGNORECASE)

    # Handle INSTR -> POSITION (Snowflake) or LOCATE (Databricks)
    if is_databricks:
    ...
    # Handle ADD_TO_DATE -> DATEADD (Snowflake) or DATE_ADD (Databricks)
    def convert_add_to_date(match):
        ...
        if is_databricks:
            if interval_name == 'DAY':
                return f"DATE_ADD({date_expr}, {num_expr})"
            else:
    ...
    def convert_date_diff(match):
        ...
        if is_databricks:
            # Databricks DATEDIFF only supports days; args are (end, start)
    ...
    # Handle TRUNC for dates -> DATE_TRUNC
    def convert_trunc(match):
        expr = match.group(1)
        interval = match.group(2) if match.group(2) else "'DAY'"
        ...
        sf_interval = interval_map.get(interval, 'DAY')
        return f"DATE_TRUNC('{sf_interval}', {expr})"
    ...
    # Handle GET_DATE_PART -> DATE_PART (Snowflake) or EXTRACT (Databricks)
    if is_databricks:
        result = re.sub(r'\bGET_DATE_PART\s*\(', 'EXTRACT(', result, flags=re.IGNORECASE)
    else:
        result = re.sub(r'\bGET_DATE_PART\s*\(', 'DATE_PART(', result, flags=re.IGNORECASE)

    # Handle TO_CHAR -> DATE_FORMAT on Databricks
    if is_databricks:
    ...
    # Handle TO_INTEGER, TO_BIGINT, TO_FLOAT
    if is_databricks:
        result = re.sub(r'\bTO_INTEGER\s*\(\s*([^)]+)\s*\)', r'CAST(\1 AS INT)', result, flags=re.IGNORECASE)
        result = re.sub(r'\bTO_BIGINT\s*\(\s*([^)]+)\s*\)', r'CAST(\1 AS BIGINT)', result, flags=re.IGNORECASE)
        result = re.sub(r'\bTO_FLOAT\s*\(\s*([^)]+)\s*\)', r'CAST(\1 AS DOUBLE)', result, flags=re.IGNORECASE)
        result = re.sub(r'\bTO_DECIMAL\s*\(\s*([^)]+)\s*\)', r'CAST(\1 AS DECIMAL)', result, flags=re.IGNORECASE)
    else:
        result = re.sub(r'\bTO_INTEGER\s*\(', 'TO_NUMBER(', result, flags=re.IGNORECASE)
        result = re.sub(r'\bTO_BIGINT\s*\(', 'TO_NUMBER(', result, flags=re.IGNORECASE)
        result = re.sub(r'\bTO_FLOAT\s*\(', 'TO_DOUBLE(', result, flags=re.IGNORECASE)

    # Handle DECODE -> CASE WHEN on Databricks (Snowflake supports DECODE natively)
    if is_databricks:
        decode_match = re.search(r'\bDECODE\s*\(', result, flags=re.IGNORECASE)
        if decode_match:
            result = _convert_decode_to_case(result)

    # Handle LOOKUP references — replace :LKP.FIELD with just FIELD (the JOIN handles the table)
    result = re.sub(r':LKP\.(\w+)', r'\1', result)

    # Handle REG_EXTRACT -> REGEXP_SUBSTR (Snowflake) / REGEXP_EXTRACT (Databricks)
    if is_databricks:
...
def generate_ddl_from_sources(sources_dict: Dict, schema: str = 'SOURCE',
    ...
    for table_name, table_info in sources_dict.items():
        ...
        using_delta = "\nUSING DELTA" if target_platform == 'databricks' else ""
